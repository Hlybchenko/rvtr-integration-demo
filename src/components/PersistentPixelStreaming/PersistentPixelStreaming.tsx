 ...
 *
 * Mounts once on first streaming device page visit and never unmounts.
...
export function PersistentPixelStreaming() {
  ...
  const iframeMounted = useStreamingStore((s) => s.iframeMounted);
  const isVisible = useStreamingStore((s) => s.isVisible);
  const viewport = useStreamingStore((s) => s.viewport);

  // Don't render anything until first streaming page visit
...
import type { StreamingViewport } from '@/stores/streamingStore';
...
const PersistentIframe = forwardRef<HTMLIFrameElement, PersistentIframeProps>(
  function PersistentIframe({ url, isVisible, viewport }, ref) {
    const iframeRef = useRef<HTMLIFrameElement>(null);
